...
class ReadData:
    ...
    def get_pandas_obj(self):
        ...
        try:
            ...
            df['site_id'] = df.apply(self.create_site_id, axis=1)
            ...
            if self.observation_type == 'daily':
                ...
                self.agg_prototype.update({'1st Max Value': self.percentile(90), 'Arithmetic Mean': 'median',
                                           'AQI': 'max', 'id': 'first'})
                df = df.groupby(['site_id']).agg(self.agg_prototype).reset_index()
                if self.measurement_type=='pm10':
...
if __name__ == "__main__":
    ...
    obj = ReadData('NO2', year='2010', county_level=True, measurement_type='no2')
    df = obj.get_pandas_obj()
    print (df.head(2))
    print(df.groupby("State Code")['id'].count())
