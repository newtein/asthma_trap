...
formula = "AC_5 ~ ev_sales+ nonev_sales  + year_fixed + (1|EPA_Region/state_code)"
model <- lmer(formula, data=data)
tidy_lmfit <- tidy(model)
wfname11 = paste(filepath, wfname1, sep="/")
write.csv(tidy_lmfit, wfname11)

ml <- lme(AC_5 ~ ev_sales+ nonev_sales  + year_fixed, random=~1|EPA_Region/state_code,data=data)
tidy_lmfit2 <- tidy(ml)
