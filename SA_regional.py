...
class MergeWithEVSales:
    ...
    def filter(self, df, col):
        ...
        bins_df = pd.read_csv("output_files/binning_at_risk_den.csv")
        select_state = bins_df[bins_df["bins"].isin([1,2,3,4])]["STATE"].tolist()
        tdf = tdf[tdf['state_code'].isin(select_state)]
